...
/// Marker resource inserted as `NonSend` so that systems requiring main-thread
/// execution can depend on it.
pub(super) struct MainThreadMarker;
...
/// Set up the macOS Settings menu bar.
///
/// Accepts a `NonSend` marker so Bevy schedules this on the main thread,
/// which is required for `AppKit` / `NSMenu` access.
pub(super) fn setup_menu_bar(mut commands: Commands, _: NonSend<MainThreadMarker>) {
    ...
    unsafe {
        ...
        add_settings_submenu(main_menu, &handler);
    }
...
unsafe fn add_settings_submenu(main_menu: *const AnyObject, handler: &MenuHandler) {
    ...
    let increase_item: *const AnyObject = msg_send![AnyClass::get("NSMenuItem").unwrap(), alloc];
    ...
    let decrease_item: *const AnyObject = msg_send![AnyClass::get("NSMenuItem").unwrap(), alloc];
